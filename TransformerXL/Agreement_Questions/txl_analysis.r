...
data <- read.delim("out_agr_qs_txl.txt",header=TRUE,sep =",")
summary(data)
...
ggplot(aes(x = VerbCondition, y = Surprisal, fill = FillerCondition), data = data) +
 geom_boxplot() +
 xlab("") + #Verb agreement across 4 levels of clausal embedding in 'which' interrogatives") +
 facet_grid(.~EmbeddingLevel) +
  theme_bw(base_size=20) + 
 theme(legend.position = "bottom") +
  ggsave("~/Desktop/txl_qs.eps",height=8,width=8)
